...
| **Heat** | Strips **4% armor** per proc; deals **50% base damage /turn** | — | **3 turns** | Both effects originate from each Heat proc |
| **Cold** | At **5 procs → Freeze** (target unusable by opponent). **0.05 increased critical damage per proc** | **Max 10** | **5 turns per proc** | Freeze activates the moment the 5th proc applies |
| **Electricity** | **50% base damage /turn** for 2 turns; **50% chance** to spread to other cards at location | — | **2 turns** | Spread applies to other enemy cards at same location |
...
| **Magnetic** | **+100% damage to shields/overguard** on first proc; **+25%** each subsequent | — | **4 turns** | On shield/overguard **break** while Magnetic present: inflict an **Electricity proc** equal to **3% of target max shields/OG per Magnetic stack** (capped at **30%**). |
| **Radiation** | Each proc: **10% chance** attacks target a **random ally**; affected cards deal **+50% damage to allied cards** | **Max 10** | **3 turns** | Encourages friendly-fire chaos; probabilities stack per proc |
...
| **Void** | Redirects projectile attacks to Void-affected card(s) | **Unlimited** | **3 turns** | If multiple Void targets exist and the chosen target is **not** Void-affected, a **random Void** card is attacked instead |
