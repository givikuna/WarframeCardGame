...
export class Blight implements IFactionSyndicateEffectMechanic {
        ...
        em.subscribe("SYNDICATE_METER_FILLED", (payload: GameEventPayload["SYNDICATE_METER_FILLED"]): void => {
            ...
            enemy.getCards().forEach((card: Card): void => {
                DamageInstance.init(
                    card,
                    this,
                    { [DamageTypePerSyndicateEffect[SyndicateEffect.Blight]]: 50 },
                    100,
                    0,
                    1,
                ).apply(owner, board);
            ...
            enemy.getOperator().takeDamage(50, DamageTypePerSyndicateEffect[SyndicateEffect.Blight]);
            owner.getOperator().heal(200);
        });
    }

    public static getFaction(): FactionSyndicate {
        return FactionSyndicate.RedVeil;
    }
}
